...
def train_schedule(days_train):
    now_m = dt_now.minute
    next_train_h = now_h
    next_train_m = 100
    after_next_train_h = now_h
    after_next_train_m = 100
    for i in range(len(days_train[now_h])):
        if days_train[now_h][i] > now_m:
            next_train_m = days_train[now_h][i]
            if i <= len(days_train[now_h])-2:
                after_next_train_m = days_train[now_h][i+1]
            else:
                after_next_train_h = now_h+1
                try:
                    after_next_train_m = days_train[now_h+1][0]
                except:
                    after_next_train_h = now_h
        ...
        if next_train_m == 100:
            ...
            next_train_m = days_train[now_h+1][0]
            if len(days_train[now_h+1]) > 1:
                after_next_train_h = now_h + 1
                after_next_train_m= days_train[now_h+1][1]
            elif len(days_train[now_h+1]) > 1:
                after_next_train_h = now_h + 2
                after_next_train_m= days_train[now_h+2][0]
    ...
    return next_train_h, next_train_m, dt1, dt2, after_next_train_h, after_next_train_m
...
if result == True or (dt_now.weekday() == 5 or dt_now.weekday() == 6):
    ...
    if now_h in [0, 1, 2, 3, 4, 5, 6]:
        ...
        next_train_h, next_train_m, dt1, dt2, after_next_train_h, after_next_train_m = train_schedule(weekend_days_train)
    ...
    if now_h in [0, 1, 2, 3, 4, 5]:
       ...
       next_train_h, next_train_m, dt1, dt2, after_next_train_h, after_next_train_m = train_schedule(working_days_train)
